...
Copy everything in the plain-text block below into the **Notes** field.
...
1) SCREEN RECORDING
A screen recording on a physical device running a recent iOS release is attached/provided with this response. It begins with launching the app and shows: Today, text capture → structured task, voice capture including system permission prompts (microphone and/or speech recognition), Notes, Calendar, Upcoming, Settings, and the Premium paywall (StoreKit).

Not applicable by design:
- No account registration, login, or account deletion (no accounts).
- No multi-user UGC, reporting, or blocking (all content is private and on-device).
- No location, contacts, camera, photo library, or App Tracking Transparency requests.

2) DEVICES / OS TESTED
- iPhone 15 Pro Max Simulator — iOS 18.x (QA / screenshots)
- iPad Pro 12.9" Simulator — iOS 18.x (QA / screenshots)
- Physical iPhone used for device/TestFlight builds — [FILL: exact model + iOS version used for the attached recording]
Minimum supported iOS: as set in the Xcode project (iOS 18.2+ per project docs).

3) PURPOSE AND AUDIENCE
LocalMind is a privacy-first task and note manager. Users capture ideas by voice or text; on-device parsing turns natural language into structured tasks (due dates, priorities, tags) and notes without a cloud account.

Problem: ideas get lost in unstructured notes/voice memos; many task apps require accounts and server storage.
Audience: privacy-conscious professionals, students, and anyone who wants fast offline-capable capture → tasks.
Value: instant capture, structured tasks/notes, calendar and upcoming views, optional local notifications, encrypted local storage, optional Premium for on-device LLM parsing and JSON export.

4) SETUP / ACCESS INSTRUCTIONS
No login. No sample account. No backend setup.

1. Launch the free app.
2. Optional: allow Microphone / Speech Recognition for voice capture.
3. Tap + Capture → enter e.g. "Submit report by Friday high priority" → Capture.
4. Browse Today, Upcoming, Notes, Calendar.
5. Settings (gear) → Upgrade for Premium.
IAP product IDs: premium_lifetime (non-consumable), premium_monthly (auto-renewable).
Both unlock the same Premium features: on-device LLM task parsing (when enabled/downloaded), JSON export, priority support.
Free tier works fully without purchase (rule-based parsing, capture, tasks, notes).
Sandbox: use a Sandbox Apple ID (App Store Connect → Users and Access → Sandbox → Testers); on device Settings → App Store → Sandbox Account.
No sample files required.

5) EXTERNAL SERVICES
Core task/note data is not sent to a developer backend (none exists). Offline-first / on-device.
- Apple StoreKit 2 — IAP/subscriptions only
- Apple Speech / Speech Recognition + Microphone — voice capture
- Optional on-device LLM model download (Premium) — model file may download over network; inference on-device; no LocalMind server for capture content
- SQLCipher — local encrypted storage
- Apple local notifications — optional reminders (no developer push server)
No developer auth service, analytics SDK, or ad network for core product data.
Privacy policy: https://www.markduenas.com/privacy

6) REGIONAL DIFFERENCES
None. Features and product IDs are the same in all regions. StoreKit shows localized prices. No geo-locked content.

7) REGULATED INDUSTRY / PROTECTED MATERIAL
Not applicable. General consumer productivity app. No regulated industry license required. No protected third-party media catalogs.
Local encryption only (SQLCipher data-at-rest). ITSAppUsesNonExemptEncryption = false (exempt local use).
```

---

## Structured reference (same content)
...
*Keep this file updated for future submissions.*
