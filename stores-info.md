...
| 2 | **Privacy policy URL** | ✅ Set | `https://www.markduenas.com/privacy` — live, verified. Added to `fastlane/metadata/ios/en-US/privacy_url.txt`. **Still needed:** paste the same URL into the Play Console Store Listing "Privacy policy" field (no local file for this on the Android side — it's a Play Console field, not fastlane metadata). Note: this is a generic umbrella policy covering all your published apps (mentions Firebase/AdMob/analytics used by *other* apps of yours), not LocalMind-specific — it covers LocalMind since you're the developer, but doesn't call out LocalMind's actual zero-analytics/zero-ads posture. Worth a quick read-through to make sure nothing there contradicts what you'll declare in the Data Safety / App Privacy forms (§6). |
| 3 | **Support URL / contact** | ✅ Set | `https://www.markduenas.com` — live, verified. Added to `fastlane/metadata/ios/en-US/support_url.txt`. Play Console will separately ask for a contact **email** on the Store Listing — use whatever inbox you actually monitor. |
